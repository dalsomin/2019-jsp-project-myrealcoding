...
public class Tour_RegistDAO {
	private static Tour_RegistDAO dao= null;
	private Tour_RegistDAO() {}
	public static Tour_RegistDAO getInstance() {
		if(dao==null) {
			dao= new Tour_RegistDAO();
		}
		return dao;
	}
	...
	public void updateProduct(Connection conn,MinbakDTO mdto){
		...
		String sql = " update product set p_name=?,p_typename=? where memberId=? ";
		...
		public void insertBnb(Connection conn, MinbakDTO mdto) {
			...
			try {
				...
				ps.setString(2, mdto.getP_name());
				ps.setInt(3, mdto.getB_atime());
				ps.setString(4, mdto.getB_hcall());
				ps.setString(5, mdto.getB_pinclusion());
				ps.setString(6, mdto.getB_tburden());
				ps.setString(7, mdto.getB_eguide());
				ps.executeUpdate();
			} catch (SQLException e) {
...
/*			} catch (SQLException e) {
		...
		public String selectpcode(Connection conn) {
				...
				String p_code =null;
				ResultSet rs = null;
				String sql = " select 't'||seq_product.currval p_code from dual";
