...
public class File_Minbak2Handler implements CommandHandler{
	...
	private String fileupload(HttpServletRequest request, HttpServletResponse response) {
		...
		String saveDirectory= request.getRealPath("/img/minbak/detail"); 
		...
		MultipartRequest mrequest = null;
			...
			uploadFile=mrequest.getFile("offerDescriptiveImageFileBtn");
	...
	//	String originalfilename = null;
		...
		//	originalfilename =mrequest.getOriginalFileName("guideProfileBtn	");
			filesystemname=mrequest.getFilesystemName("offerDescriptiveImageFileBtn");
	...
	Member member = (Member) request.getSession().getAttribute("authUser");
		...
		MinbakService ms =new MinbakService();
		...
		if (p_code.equals("first")) {
			...
			jsonArray=ms.insert_bnb_dpic(p_code,filesystemname,c_code);
