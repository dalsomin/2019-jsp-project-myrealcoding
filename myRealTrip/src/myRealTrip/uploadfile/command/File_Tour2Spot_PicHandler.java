...
public class File_Tour2Spot_PicHandler implements CommandHandler{
	...
	private String fileupload(HttpServletRequest request, HttpServletResponse response) {
		...
		String saveDirectory= request.getRealPath("/img/tour/spot_pic"); 
		...
		MultipartRequest mrequest = null;
			...
			uploadFile=mrequest.getFile("msp_picture_input");

		String filesystemname  = null;
		
		if( uploadFile != null ) { // 첨부파일이 있는 경우에만.
			long uploadFile_length = uploadFile.length();
			filesystemname=mrequest.getFilesystemName("msp_picture_input");
			System.out.println("| filename-----1"+filesystemname);
		}
		System.out.println("| filename----2"+filesystemname);
		...
		tdto.setMSP_PICTURE(filesystemname);
		...
		System.out.println("| sysp_code-----"+tdto.getP_CODE());
		...
		if (ispic.equals("firstpic")) {
			...
			jsonArray=ts.tempfileInsertnSelect(tdto);
			System.out.println(jsonArray);
		}else {
			System.out.println("처음이 아니시군요...");
			jsonArray=ts.tempfileUpdatenSelect(tdto);
			System.out.println(jsonArray);
		}
