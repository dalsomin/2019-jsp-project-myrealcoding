...
<!-- Title for the page -->
<title> 모든 티켓 : My Real Trip </title>
...
<meta content="2Et40MsZgluscJFLQm0B6IXt7RtOanlfaD9+GYwQ+jw=" name="csrf-token" />
		...
		<link href="https://assets4.freshdesk.com/assets/cdn/portal_print-ce4892062f25ebaf8abdcca87f6225b0.css" media="print" rel="stylesheet" type="text/css" />
	  		<link href="https://assets6.freshdesk.com/assets/cdn/falcon_portal_utils-fc010ba819d2d80256daa9858dbbc765.css" media="screen" rel="stylesheet" type="text/css" />	
		...
		<script src="https://assets9.freshdesk.com/assets/cdn/portal_head-cc545c114f8f15358aa7acd4b9de11a5.js" type="text/javascript"></script>
...
<div class="layout layout--logged-in  layout--ticket_list">
  ...
  <div class="dropdown" id="ticket-filters" >  
  	...
  	<a href="<%= request.getContextPath() %>/myRealTrip/support/solution/qnaListAjax.do" class="list-lead dropdown-toggle" data-toggle="dropdown">
  	모든 티켓 <i class="caret"></i>
  	</a>
   <ul class="dropdown-menu " role="menu" aria-labelledby="dropdownMenu" id="tfdiv">
     <li class="selected" value="1"><a href="<%= request.getContextPath()%>/myRealTrip/support/solution/qnaListAjax.do"  >모든 티켓</a></li>
    
  <li  value="2" ><a href="<%= request.getContextPath()%>/myRealTrip/support/solution/qnaListAjax.do" >열림 또는 대기 중</a></li>
     <li  value="3"><a href="<%= request.getContextPath()%>/myRealTrip/support/solution/qnaListAjax.do" >해결됨 또는 닫힘</a></li> 
  </ul>
  ...
  <div id="ticket-sub-toolbar">
    <span id="ticket-sort" class="dropdown">
      ...
      <li class="selected" ><span class='icon ticksymbol'></span>
      <a href="/support/tickets/filter?wf_order=created_at" data-loading-box="#ticket-list" data-method="get" data-remote="true" data-response-type="script">작성 일자</a>
      </li>
      <li>
      <a href="/support/tickets/filter?wf_order=updated_at" data-loading-box="#ticket-list" data-method="get" data-remote="true" data-response-type="script">마지막 수정</a>
      </li>
      <li>
      <a href="/support/tickets/filter?wf_order=status" data-loading-box="#ticket-list" data-method="get" data-remote="true" data-response-type="script">처리 상태</a></li>
      <li class="divider"></li>
      <li><a href="/support/tickets/filter?wf_order_type=asc" data-loading-box="#ticket-list" data-method="get" data-remote="true" data-response-type="script">증가</a></li>
      <li class="selected" ><span class='icon ticksymbol'></span><a href="/support/tickets/filter?wf_order_type=desc" data-loading-box="#ticket-list" data-method="get" data-remote="true" data-response-type="script">감소</a></li></ul>
      ...
      <div id="ticket-list">
		<div class="c-list">
		   ...
		   <div class="c-row c-ticket-row">
			    ...
			    <c:if test="${ qna.ql_s_code eq 3 }">
					<span class="label-status-pending label label-small" style="color:#0d9074">
						${ qna.ql_s_detail }
					</span>
					</c:if>
			    <c:if test="${ qna.ql_s_code ne 3 }">
					...
					<div class="ticket-brief">
					    <div class="ellipsis">
					      	<a href="/myRealTrip/myRealTrip/support/solution/qArticles.do?cq_code=${ qna.cq_code }&qq=qq" class="c-link" title="Feedback for:  여행일 변경 혹은 인원 변경 가능한가요?  추가 정보 필요   d">Article Feedback - ${qna.cq_title } #${qna.cq_code }</a> 
					    </div>
					    <div class="help-text">
								<span class='emphasize'>${ qna.cq_date }</span>에 생성됨

						       에이전트: <span class='emphasize'>김태형</span>
						</div>
 ...
 */
  ...
  /* ajax!!!! */  
    ...
    $("ul#tfdiv a").click(function (){
    	//alert("click")
    //	var url = $(this).prop("href");
    	//alert( url )
    	$("ul#tfdiv li").removeClass("selected");
    	$(this).parent().addClass("selected");
    	
    	   url = $(this).prop('href');	  
    	//  alert(url);
    	  var ql_s_code2  = $(this).parent().val(); //2
    	//  alert(ql_s_code2);
     
   var pp = {
		   cq_email: "aaa@naver.com" ,
		   ql_s_code: ql_s_code2
         };
   //?ql_s_code = 2(ql_s_code2)
       
     $.ajax({
    		  url : url, 
    		  dataType : "json",
    		  type : "get",
    		  //data: {ql_s_code: ql_s_code},
    		  data : pp,
    		  cache : false,
    		 ...
    		 // alert(data);
    		        ...
    		        $(data).each(
							function( index, e ) {
								...
								var cq_title = e.cq_title;
								var ql_s_detail = e.ql_s_detail;
								var cq_date = e.cq_date;
								var ql_s_code = e.ql_s_code;
								var cq_code = e.cq_code;
								var list = $("<div />").html(cq_title);
							
								 var list = "<div class='c-row c-ticket-row'><span class='status-source sources-detailed-portal' title='포털 채널을 통해 보고됨'> </span>";
								 
								 if (ql_s_code == 3) {
									list += "<span class='label-status-pending2 label label-small labelstyle' style='color:#0d9074'>";
								}else{
									list += "<span class='label-status-pending label label-small labelstyle'>";
									
								} 
							    /* "<span class='label-status-pending label label-small labelstyle'>"+  */
								 list += ql_s_detail+"</span> <div class='ticket-brief'> <div class='ellipsis'>"
							     +" <a href='/myRealTrip/myRealTrip/support/solution/qClickedArticle.do?cq_code="+cq_code+"' class='c-link' title='Feedback for:  여행일 변경 혹은 인원 변경 가능한가요?  추가 정보 필요   d'>Article Feedback - "
							    	+ cq_title + " #"+cq_code+" </a>" 
							     +"  </div>"
							     +" <div class='help-text'><span class='emphasize'>"
							     + cq_date + "</span>에 생성됨 에이전트: <span class='emphasize'>김태형</span></div>"
								 +"</div> </div>"
								...
								/* 
							if(ql_s_code == 3){
								$(".label-status-pending").css("color","#0d9074");
							}
								   */
		...
		<script src="https://assets8.freshdesk.com/assets/cdn/lang/ko-c62c7d023f5cecf863628fa3fce02dd5.js" type="text/javascript"></script>
